WTW_3DINTERNET.prototype.addMoldMyCustomMold = function(zmoldname, zmolddef, zlenx, zleny, zlenz) {
	let zmold;
	try {
		...
		WTW.log("plugins:wtw-3dinternet:scripts-custom_molds.js-addMoldMyCustomMold=" + ex.message);
	}
	return zmold;
}

WTW_3DINTERNET.prototype.setNewMoldDefaults = function(zshape, zpositionX, zpositionY, zpositionZ, zrotationY) {
	try {
		...
		WTW.log("plugins:wtw-3dinternet:scripts-custom_molds.js-setNewMoldDefaults=" + ex.message);
	}
}

WTW_3DINTERNET.prototype.setMoldFormFields = function(zshape) {
	try {
		/* add each custom mold to this one function as a case - no need to add additional hooks */
		/* zshape is name of my custom mold - all lowercase and no spaces */
		switch (zshape) {
		...
		WTW.log("plugins:wtw-3dinternet:scripts-custom_molds.js-setMoldFormFields=" + ex.message);
	}
}
